...
- `/out/:token/guide.json` emits only the enabled output's enabled member channels and programmes overlapping a caller-supplied UTC window. The window is limited to 48 hours, 2,000 channels, 5,000 emitted programme entries, and 8 MiB of JSON; public description summaries are capped at 1,000 Unicode characters. Duplicate imports are collapsed, ordering is deterministic, and oversized results fail instead of being truncated. Channels without an EPG mapping remain in the response with an empty programme list.
