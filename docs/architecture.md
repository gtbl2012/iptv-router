...
HTTP(S) sources are probed with a bounded range read. Each `health_check` records status, HTTP code, latency, throughput, bytes read, a redacted error code, and time until the configured retention window expires; the source row holds the current projection. Successful samples are also passed from memory to an optional ffmpeg process for one bounded JPEG preview frame, without giving ffmpeg a network URL. Non-HTTP protocols are retained as `unknown` instead of being falsely marked offline.
...
- `/out/:token.m3u` emits deterministic extended M3U with sanitized metadata and router-owned stream URLs.
